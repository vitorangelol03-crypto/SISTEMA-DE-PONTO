...
test.describe('Pagamentos Driver — busca por nome nos modais', () => {
  ...
  test('espelhos recebidos: filtra enquanto digita e ignora acento', async ({ page }) => {
    ...
    await page.getByRole('button', { name: /Notas recebidas/ }).first().waitFor({ timeout: 30_000 });
    await page.getByRole('button', { name: /Espelhos recebidos|Prints recebidos/ }).first().click();
