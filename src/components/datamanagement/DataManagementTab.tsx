      ...
      for (const dataType of selectedDataTypes) {
        ...
        if (dataType === 'attendance') {
          ...
          formattedData = rawData.map(item => ({
            ...
            'CPF': item.employees?.cpf || 'N/A',
            'Data': format(new Date(item.date + 'T00:00:00'), 'dd/MM/yyyy'),
            'Tipo': (item.error_type ?? 'quantity') === 'value' ? 'Valor (R$)' : 'Quantidade',
            'Quantidade de Erros': (item.error_type ?? 'quantity') === 'quantity' ? item.error_count : '-',
            'Valor do Erro (R$)': item.error_type === 'value' ? Number(item.error_value ?? 0).toFixed(2) : '-',
