...
> apareceu foi pior que "não confirmada".
> 🔴 **INSS: 3 das 4 faixas e o teto errados.** Salário 3.000 descontava 245,68 (devia
> 248,60); 9.500 descontava 1.070,67 quando o teto é 988,08. **Até R$ 2.902 não havia
> erro** — exatamente onde os 11 recibos do gabarito paravam (maior salário R$ 2.200).
> 🔴 **IRRF sem a redução da Lei 15.270/2025:** cobrava imposto de quem a lei isenta desde
> janeiro (R$ 312,89 de quem ganha R$ 5.000, R$ 114,76 de quem ganha R$ 4.000).
> 🎯 **E o achado maior: O MÉTODO.** Ao trocar pela tabela oficial, **o gabarito dos 11
> recibos reais ficou VERMELHO em 4**. A tabela publica dois jeitos de calcular que
> discordam em 1 centavo, porque a "parcela a deduzir" publicada é arredondada (24,315 →
> 24,32). Somando faixa a faixa: 4 erros. Com a parcela a deduzir: **0 de 11**. A
> contabilidade usa a parcela. Isso também explicou o `1.621,30` da tabela antiga: não era
> valor real, era o limite **entortado** pra fazer o método errado imitar o certo.
> 🔴 **A LIÇÃO: bater com o gabarito não é estar certo — é estar certo no pedaço que o
> gabarito cobre.**
> ✅ **Gabaritos que batem agora:** 11/11 recibos reais + o exemplo oficial da Receita
> (Rita, R$ 6.000 → 562,63 − 179,75 = **382,88**, no centavo).
> ✅ **A redução só sai na folha do MÊS** e só se quem chama pedir (`incidenciaMensal`); o
> padrão é NÃO reduzir. 13º e rescisão fora — decisão do Victor, porque reduzir por engano
> faz recolher imposto a menos. O recibo diz quanto foi abatido.
> ✅ **3 migrations** (`20260921092418`, `...092748`, `...095508`). `confirmado` segue
> FALSO e a tarja continua no recibo.
> ✅ **118 arquivos / 1.858 unitários · E2E folha 37/37 · tsc · lint · build.**
> 📋 **Pendência afiada pro contador:** as tabelas conferem? a redução vale no 13º e no
> saldo da rescisão? no teto vale 988,08 (fórmula) ou 988,09 (publicado)?
> Detalhe em `CHECKPOINT_SESSAO_2026-09-21.md`.
...
> mensal) e **cada verba numa linha** — decisões do Victor.
> ✅ **`papelGravado.ts`** lê o que foi guardado e **nunca recalcula**; sem o `papel`,
> monta o mínimo dos valores (o detalhe se perde, o dinheiro não).
> 🔴 **Achado gerando o relatório:** em dezembro a pessoa pode ter o 13º do ano E a
> rescisão, e saía **"INSS sobre 13º" duas vezes** — parece duplicação. O da rescisão
> virou "INSS sobre 13º **proporcional**".
> ⚠️ **Erro meu na validação da leva 5:** o laço de lotes ia até o 9º e o `split` criava
> 10 — disse "109 arquivos" tendo rodado 108. O que faltou está verde; a contagem é que
> estava errada. **Conferir `ls lote_* | wc -l` contra o laço.**
> ✅ **110 arquivos / 1.762 unitários** · E2E **121 1/1** · sem regressão (14). Provado no
> PDF: 16.990,54 − 203,05 = 16.787,49 com os dois juntos.

> **📌 SESSÃO 19/09 (leva 5) — 2ª VIA DO 13º E DA RESCISÃO.** Commit `9650eb7`.
>
> 🔴 **Ponta minha:** as tabelas guardavam o acerto "para reimprimir depois" (diz assim
> na tela), mas **não existia tela que reimprimisse**.
> 🔴 **E o 13º até dava pra reconstruir dos valores; a RESCISÃO não** — os dias e avos da
> coluna de referência nunca eram gravados. Papel parecido ≠ o mesmo papel, e numa
> rescisão isso é grave.
> ✅ **Migration `20260920023702`** (com OK): coluna `papel` (jsonb) nas duas tabelas,
> guardando o cálculo INTEIRO como saiu. A 2ª via é **releitura, nunca recálculo**.
> ✅ O papel sai marcado **"— 2ª VIA"**: duas vias circulando como original é o que não
> pode existir.
> 🎯 **A prova:** o E2E registra a rescisão com salário 1.700, **muda para 2.600**, e
> reimprime — o PDF sai com o líquido ORIGINAL, lido de dentro do arquivo.
> ✅ **109 arquivos / 1.744 unitários** · E2E **120 2/2** · sem regressão. Deploy 3/3.
> ⏳ **Próximo candidato:** os relatórios mostrarem 13º e rescisão (hoje só a folha mensal).
...
>
> ✅ **13º:** as duas opções de parcela (1ª+2ª ou única), base = salário + média do
> adicional noturno do ano, avos pela regra dos 15 dias. Tela no Financeiro com
> **Baixar** (não grava) e **Registrar** (grava) separados. Migration `20260919220837`
> (`payroll_thirteenth`) **aplicada com OK do Victor** e provada por catálogo + 3
> simulações que se desfizeram. Papel provado: **857,72 + 727,66 = 1.585,38**.
> ✅ **Férias por avos:** período aquisitivo, avos no **relógio da admissão** (não no
> calendário), tabela de faltas do art. 130, e o alerta de **férias VENCIDA no topo** —
> que a lei manda pagar em dobro. **Sem migration.**
> 🔴 **Erro de desenho pego por teste vermelho:** o saldo somava o período fechado com o
> proporcional em curso — a tela deixaria agendar 40 dias pra quem pode tirar 30.
> Separado em "pode tirar" e "em formação".
> ⚠️ **BLOQUEIA O USO REAL: 18 das 21 fichas de carteira assinada estão SEM data de
> admissão**, e sem ela não há período aquisitivo. O "primeiro ponto" não serve de
> atalho — 11 pessoas têm o primeiro ponto no mesmo 06/11/2025 (o dia em que o sistema
> começou), e são as mais antigas. **Victor precisa preencher as 18 datas.**
> ⚠️ **Sem gabarito:** os 12 recibos da contabilidade são de julho e nenhum tem 13º.
> Sai com a tarja "VALORES EM CONFERÊNCIA", como o IRRF.
> ✅ Validado: typecheck 0 · lint 0 · build limpo · **106 arquivos / 1.707 unitários** ·
> E2E **117 5/5** e **118 4/4** novos · sem regressão. Deploy conferido por conteúdo.
> 🔧 Dois consertos fora do escopo: o `holeriteLotePdf` piscava 1 em 400 (comparava o
> relógio dentro do PDF — hora fixada, 0 em 3.000 depois) e uma variável sem uso no
> `tests/117` que **escapou no commit do 13º** (rodei o lint antes de escrever o spec).
> ⏳ **Falta a leva 4: RESCISÃO** — depende das duas de cima.
...
>
> ✅ **VALIDADO E NO AR:** suíte **101 arquivos / 1.527 testes** verde; E2E verdes nos
> specs tocados (06 · 11 · 22 · 26 · 35 · 38 · 100); `public-api-v1` republicada na
> **v6** (pedia a coluna removida → 500, pego pelo teste); deploy conferido **byte a
> byte** em 14/09. ⚠️ As telas são `lazy`: a feature nova NÃO aparece no bundle
> principal — conferir o chunk da tela. ⏳ **E2E 02, 15 e 47 ainda não rodaram.**
>
...
> R$ 15.980,60 do grupo — **mas os dois no mesmo nome**. 🔴 **Causa, lida nos 2 PDFs:**
> as duas notas foram emitidas pelo MESMO prestador (Joaerson, CNPJ 55.857.717/0001-46);
> o que mudava era o TOMADOR (Shopee e CD Logística). O `nota_emitter_id` que a edge fn
> comparava **é o CNPJ do tomador** — nunca existiu comparação de quem EMITE, apesar de a
> tela prometer isso desde 05/09. Ou seja: o teto por CNPJ, motivo da feature existir,
> não estava sendo respeitado. **Fechado:** coluna nova `matched_cnpj` (migration
> `20260907120000`), `matchedCnpjs` no `nfCheck`, e a 2ª nota do mesmo CNPJ da 1ª passa a
> ser **recusada** dizendo qual CNPJ falta (sem CNPJ dos dois lados **não** recusa — nunca
> no escuro). A tela do app agora **nomeia os dois emissores** em vez do genérico "outro
> CNPJ", o cartão da 2ª diz quem tem que emitir, e dividir só aparece pra quem tem **2
> CNPJs** cadastrados (medido antes: só o Gessiley tem cadastro e ele já tem os 2 —
> ninguém perde a opção). O relatório passa a achar a chave PIX **pelo CNPJ** da nota
> (nome de reserva pras antigas). Chaves PIX do Joaerson e do Gessiley cadastradas, tiradas
> do cadastro que já existia na ficha — a NFS-e não tem campo bancário. **Decisão do
> Victor: as notas do Gessiley que já passaram ficam como estão; a trava vale da próxima
> dupla em diante** (o pagamento desta quinzena sai nas 2 linhas do Joaerson). Provado com
> **E2E `tests/107` 5/5 verde** contra a edge fn deployada (v42 → **v43**), com o teste E
> novo que só passa com a trava no ar; typecheck 0, lint 0, build limpo, **1361 unitários**.
> Vercel conferida pelo conteúdo do bundle no ar.
>
> Atualização anterior — **2026-09-05** — **NOTA FISCAL DIVIDIDA reescrita de ponta a ponta**
